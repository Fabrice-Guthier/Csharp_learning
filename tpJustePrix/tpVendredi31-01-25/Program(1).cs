        ...
        static void Main(string[] args)
        {
            ...
            while (continueGame == "o")
            {
                ...
                if (difficulty == 1)
                {
                    ...
                    int easyModeNumber = randomNumber.Next(1, 100);
                    while (tryCount <= maxTry && !targetNumber)
                    {
                        Console.WriteLine("Vous avez " + (maxTry - tryCount) + " essais pour trouver le juste prix, dans une tranche de 1 á 100 :");
                ...
                else if (difficulty == 2)
                {
                    ...
                    int mediumModeNumber = randomNumber.Next(1, 1000);
                    while (tryCount <= maxTry && !targetNumber)
                    {
                        Console.WriteLine("Vous avez " + (maxTry - tryCount) + " essais pour trouver le juste prix, dans une tramche de 1 á 1000 :");
                        inputNumber = Console.ReadLine();
                        number = int.Parse(inputNumber);
                        tryCount++;
                ...
                else if (difficulty == 3)
                {
                    ...
                    int hardModeNumber = randomNumber.Next(1, 10000);
                    while (tryCount <= maxTry && !targetNumber)
                    {
                        Console.WriteLine("Vous avez " + (maxTry - tryCount) + " essais pour trouver le juste prix, dans une tranche de 1 á 10000 :");
                        inputNumber = Console.ReadLine();
                        number = int.Parse(inputNumber);
                        tryCount++;
